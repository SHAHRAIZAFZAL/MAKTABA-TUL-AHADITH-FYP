...
$indexID = htmlspecialchars($_GET["indexID"]);
...
$stmt = $conn->prepare("SELECT * FROM $fullTableName WHERE index_id = ?");
...
$baabNoteAr = htmlspecialchars($row['baab_note_ar']);
$baabNoteEn = htmlspecialchars($row['baab_note_en']);
$baabNoteUr = htmlspecialchars($row['baab_note_ur']);

$result = $conn->query("SELECT MAX(index_id) as max_id FROM $fullTableName");
$row = $result->fetch_assoc();
$indexCount = $row["max_id"];
$prevIndex = $indexID - 1;
$nextIndex = $indexID + 1;
?>
<!DOCTYPE html>
            ...
            <div class="main">
                ...
                <article class="container container-without-pad-bg">
                    <form action="" method="get" class="quran-list prev-next">
                        <a href="<?php echo BASE_URL . "hadith/hadith-book-content-chapter.php?hadithBookName=" . $bookLink . "&indexID=" . $prevIndex ?>"
                            class="quran-surah
                    <?php if ($prevIndex == 0)
                        echo "disable-clicks less-opacity\" tabindex=\"-1" ?>">
                                <span class="langDiv langDiv-en fontDiv fontDiv-s">Prev. Chapter</span>
                                <span class="langDiv langDiv-ur urdu-text hide-imp fontDiv fontDiv-m">پچھلا باب</span></a>
                            <a href="<?php echo BASE_URL . "hadith/hadith-book-content-chapter.php?hadithBookName=" . $bookLink . "&indexID=" . $nextIndex ?>"
                            class="quran-surah
                    <?php if ($nextIndex > $indexCount)
                        echo "disable-clicks less-opacity\" tabindex=\"-1" ?>">
                                <span class="langDiv langDiv-en fontDiv fontDiv-s">Next Chapter</span>
                                <span class="langDiv langDiv-ur urdu-text hide-imp fontDiv fontDiv-m">اگلا باب</span></a>
                        </form>
                    </article>
                    ...
                    if ($baabNoteAr) {
                        ...
                        echo '<p class="arabic-text fontDiv fontDiv-ml">' . $baabNoteAr . '</p> <p class="noteToShowHide english-text hide fontDiv fontDiv-m">' . $baabNoteEn;
                        echo '</p> <p class="noteToShowHide urdu-text hide fontDiv fontDiv-ml">' . $baabNoteUr . '</p> </div> </article>';
                    }

                    $newTableName = substr($fullTableName, 0, -6);
                    $stmt = $conn->prepare("SELECT * FROM $newTableName WHERE hadith_id >= ? AND hadith_id <= ?");
                    $stmt->bind_param("ii", $baabRangeUp, $baabRangeDown);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    $stmt->close();
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $hadithGrade = htmlspecialchars($row['hadith_grade']);
                        $hadithGradeNotes = htmlspecialchars($row['hadith_grade_notes']);
                        $hadithTakhreej = htmlspecialchars($row['hadith_takhreej']);
                        $hadithAr = htmlspecialchars($row['hadith_ar']);
                        ...
                        foreach ($translatorArray as $element) {
                            $t = explode(": ", $element);
                            $hadithTranslations[$t[1]] = htmlspecialchars(($row[$t[1]]));
